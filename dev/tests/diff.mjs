...
report.mainHtml = mainA === mainB ? 'identical' : squash(mainA) === squash(mainB) ? 'identical except whitespace between tags' : { differs: firstDiff(mainA.replace(/></g, '>\n<'), mainB.replace(/></g, '>\n<')) };
...
for (const w of Object.keys(sA.full)) {
  ...
  for (let i = 0; i < pb.data.length; i += 4) {
    const d = pa.data[i] !== pb.data[i] || pa.data[i + 1] !== pb.data[i + 1] || pa.data[i + 2] !== pb.data[i + 2];
    if (d) { n++; rows.add(Math.floor(i / 4 / pb.width)); out[i] = 255; out[i + 1] = 0; out[i + 2] = 0; out[i + 3] = 255; }
  ...
  report.pixels[w] = n ? `${n} differing pixels, rows ${ys[0]}–${ys.at(-1)}` : '0 differing pixels';
}
