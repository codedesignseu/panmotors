...
**D10 (review, 24 Sep 2026):** Our Values and Live are not pages. Values live on the About page (and show on home as designed, cards linking to About). Live is a homepage-only section, its posts entered on Home. The hero has no sound button. The homepage matches `_design/index.html` exactly: nothing is added to it that the design does not have (no "view all" links). Inner pages stay placeholders until they are designed.

 Each primary menu item is its own page. This file supersedes every part of `theme-map.md` and `CLAUDE.md` that assumes a single page with anchor links (`#floor`, `#ways`, etc.). Still no car pages, no car post type, no prices (D1 stands).
...
| Values repeater | About page | the 3 cards, each linking to the About page |
...
| Live posts repeater, eyebrow, title, Instagram pill label | Home page (home-only section) | the posts grid |
...
- `context = home`: section exactly as in the design, with its H2. No extra links or elements (D10).
...
- Components: `featured-cars`, `values`, `about`, `latest-cars`, `live` (home only), `showroom`, `hours`, `faq`, `enquire`, `cta-band`.
- The existing hero and marquee stay home-only in `template-parts/front/`.
- Only the `home` context is built until the inner pages are designed.
...
- **About**: page hero, the one-sentence description as the lead paragraph, `about_story` (wysiwyg, the long version), image, stats, Our Values (the values repeater, with optional `detail` and `image` per value), marques list (from options, as a plain list, not the marquee), CTA band. The scroll colour fade stays on the home About section only.
